...
public class CheckSigInvalidVotes extends AbstractVerification {
	...
	public CheckSigInvalidVotes(final PathService pathService, final CertificateLoader certificateLoader,
	...
	public VerificationDefinition getVerificationDefinition() {
		...
		definition.setDescription(
				TranslationHelper.getFromResourceBundle(Block4VerificationSuite.RESOURCE_BUNDLE_NAME, "verification76.description"));
		definition.setId(76);
		definition.setName("checkSigInvalidVotes");
	...
	public VerificationResultEvent verify(final VerifierEvent event) {
		...
		final var invalidVotesPathNode = pathService.buildFromRootPath(StructureKey.INVALID_VOTES_RESULT, inputDirectoryPath);

		// Verify signature of each file
		for (final Path invalidVotesPath : invalidVotesPathNode.getRegexPaths()) {
			final byte[] content;
			final byte[] signature;
			try {
				content = Files.readAllBytes(invalidVotesPath);
				signature = Files.readAllBytes(invalidVotesPathNode.getRelation(RelationType.METADATA, invalidVotesPath));
			} catch (IOException e) {
				throw new UncheckedIOException("Failed to read invalid votes results or its metadata.", e);
			}
