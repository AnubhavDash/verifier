...
public class CheckSigInvalidVotesDecryption extends AbstractVerification {
    ...
    public VerificationResult verify(File inputDirectory) {
        ...
        try {
            ...
            result.setMessage(TranslationHelper.getFromResourceBundle(Block4VerificationSuite.RESOURCE_BUNDLE_NAME, "verification78.nok.message"));

        } catch (NoSuchFileException e) {
            LOGGER.error("a NoSuchFileException error occurred", e);
            result.setStatus(Status.NOK);
            result.setMessage(TranslationHelper.getFromResourceBundle(Block4VerificationSuite.RESOURCE_BUNDLE_NAME, "verification78.file.not.found.message", e.getFile()));
