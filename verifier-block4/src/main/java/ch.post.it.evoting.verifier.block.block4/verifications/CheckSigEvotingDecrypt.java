...
public class CheckSigEvotingDecrypt extends AbstractVerification {
    ...
    public VerificationResult executeVerification(File inputDirectory) {
        ...
        try {
            ...
            result.setMessage(TranslationHelper.getFromResourceBundle(Block4VerificationSuite.RESOURCE_BUNDLE_NAME, "verification72.nok.message", e.getArgs()));
        } catch (NoSuchFileException e) {
            LOGGER.error("a NoSuchFileException error occurred", e);
            result.setStatus(Status.NOK);
            result.setMessage(TranslationHelper.getFromResourceBundle(Block4VerificationSuite.RESOURCE_BUNDLE_NAME, "verification72.file.not.found.message", e.getFile()));
        } catch (Exception e) {
