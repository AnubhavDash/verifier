...
public class CheckSigEvotingDecrypt extends Verification {

    private static final Logger LOGGER = Logger.getLogger(CheckSigEvotingDecrypt.class);
    ...
    public VerificationDefinition getVerificationDefinition() {
        ...
        def.setDescription(TranslationHelper.getFromResourceBundle(Block4VerificationSuite.RESOURCE_BUNDLE_NAME, "test72.description"));
        def.setId(72);
    ...
    public VerificationResult executeVerification(File inputDirectory) {
        ...
        try {
            ...
            if (!SignatureChecker.verifyPKCS7(content, signature, rootCertificate)) {
                throw new VerificationFailureException(evotingDecrypt.getName());
            }
            result.setStatus(Status.OK);

        } catch (VerificationFailureException e) {
            result.setStatus(Status.NOK);
            result.setMessage(TranslationHelper.getFromResourceBundle(Block4VerificationSuite.RESOURCE_BUNDLE_NAME, "test72.nok.message", e.getArgs()));
        } catch (NoSuchFileException e) {
            LOGGER.error("a NoSuchFileException error occurred", e);
            result.setStatus(Status.NOK);
            result.setMessage(TranslationHelper.getFromResourceBundle(Block4VerificationSuite.RESOURCE_BUNDLE_NAME, "test72.file.not.found.message", e.getFile()));
        } catch (Exception e) {
