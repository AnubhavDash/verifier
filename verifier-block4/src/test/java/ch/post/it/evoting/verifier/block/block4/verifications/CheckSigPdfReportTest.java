...
public class CheckSigPdfReportTest extends Block4VerificationAbstractTest {
    ...
    @Test
    public void executeTestNOKFileNotFoundReportResult() throws Exception {
        exceptionRule.expect(NoSuchFileException.class);
        exceptionRule.expectMessage(matchesRegex(verification.getPathService().getStructureNode(StructureKey.REPORT_RESULT).getQualifier()));
        verification.verify(Paths.get(getClass().getResource("/CheckSigPdfReportTest/NOK-NOTFILE2").toURI()));
    }

    @Test
    public void executeTestNOKFileNotFoundReportResultP7() throws Exception {
        exceptionRule.expect(NoSuchFileException.class);
        exceptionRule.expectMessage(matchesRegex(
                verification.getPathService().getStructureNode(StructureKey.REPORT_RESULT).getQualifier()
