...
public class CheckSigConfig extends AbstractVerification {
	...
	public CheckSigConfig(final PathService pathService, final CertificateLoader certificateLoader,
	...
	public VerificationDefinition getVerificationDefinition() {
		...
		definition.setId(73);
		definition.setName("checkSigConfig");
	...
	public VerificationResultEvent verify(final VerifierEvent event) {
		...
		final var configAnonymizedPathNode = pathService.buildFromRootPath(StructureKey.CONFIG_ANONYMIZED, inputDirectoryPath);
		final byte[] content;
		final byte[] signature;
		try {
			content = Files.readAllBytes(configAnonymizedPathNode.getPath());
			signature = Files.readAllBytes(configAnonymizedPathNode.getRelation(RelationType.P7));
		} catch (IOException e) {
			throw new UncheckedIOException("Failed to read config anonymized or its signature file.", e);
		}
		...
		if (!SignatureChecker.verifyPKCS7(content, signature, rootCertificate)) {
			return VerificationResultEvent.failure(this, getVerificationDefinition(),
					TranslationHelper.getFromResourceBundle(Block1VerificationSuite.RESOURCE_BUNDLE_NAME, "verification73.nok.message",
