...
public class VerifyElGamalParametersPQ extends AbstractVerification {
	...
	private static IntPredicate givesValidValues(int N, byte[] seedDecodedBytes) {
		return counter -> {
	...
	private static BigInteger computeQ(int N, byte[] seedDecodedBytes, int counter) {
	...
	private static void truncate(final int digestBitLength, final BitSet bitSet) {
