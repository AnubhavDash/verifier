...
import static ch.post.it.evoting.verifier.common.block.tools.Requirements.requireIsInZ_q;
import static ch.post.it.evoting.verifier.common.block.tools.Requirements.requireIsMember;
import static ch.post.it.evoting.verifier.common.block.tools.Requirements.requireVectorDimensionEqual;
import static ch.post.it.evoting.verifier.common.block.tools.Requirements.requireVectorDimensionLTE;
import static ch.post.it.evoting.verifier.common.block.tools.Requirements.requireVectorIsInZ_q;
import static ch.post.it.evoting.verifier.common.block.tools.Requirements.requireVectorIsMember;
...
public final class MathHelper {
	...
	/**
	 * Utility to verify membership for encryptionParameters.
	 *
	 * @param x                    A number
	 * @param encryptionParameters the associated encryption parameters
	 * @return true if <code>x &isin; encryptionParameters</code>, false otherwise
	 */
	public static boolean isMember(BigInteger x, EncryptionParameters encryptionParameters) {
		return isMember(x, encryptionParameters.getP());
	}
	...
	public static boolean isMember(BigInteger x, EncryptionGroup encryptionGroup) {
		return isMember(x, encryptionGroup.getP());
	}

	private static boolean isMember(BigInteger x, BigInteger p) {
		return isGTE(x, BigInteger.ONE) && isLT(x, p) && JacobiSymbol.computeJacobiSymbol(x, p) == 1;
	}
	 ...
	 * <p>
	 * This primitive is used to map an input value <code>∈ [0, 1]<sup>𝑛</sup></code> to a random-looking value <code>∈ ℤ𝑞</code>. This is used by
	 * the proofs relying on the Fiat-Shamir heuristic to turn an interactive protocol into a non-interactive proof system.
	 *
