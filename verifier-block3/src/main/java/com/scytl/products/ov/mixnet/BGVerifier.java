...
public class BGVerifier {
    ...
    public static boolean verify(final Path outputParentPath) throws VerifierException {
        ...
        final File[] files = outputParentPath.toFile().listFiles(File::isDirectory);
        if (files != null) {
            for (File file : files) {
                ...
                if (!batchName.equals("tally")) {
                    ...
                    try {
                        ZpGroup zpGroup = BGReader.createZpGroup(outputParentPath, batchName);
                        ElGamalPublicKey publicKey = BGReader.createElGamalPublicKey(batchName, outputParentPath);
                        ...
                        final ElGamalEncryptedBallots encryptedBallots =
                            ElGamalEncryptedBallotsLoader.loadCSV(zpGroup.getParams(), outputParentPath, batchName,
                        ...
                        final ElGamalEncryptedBallots reencryptedBallots = ElGamalEncryptedBallotsLoader.loadCSV(
                            zpGroup.getParams(), outputParentPath, batchName,
                        ...
                        final ShuffleProof shuffleProof = proofsReader.read(outputParentPath, batchName);

                        final ShuffleProofVerifier shuffleProofVerifier = getVerifier(zpGroup, cryptosystem,
                            shuffleProof, outputParentPath, batchName, encryptedBallots, reencryptedBallots);
     ...
     * 
     * @param shuffleProof
     *            The bean containing the output of a shuffle proof
     * @param outputParentPath
     *            Path to obtain the configuration values
     * @param batchName
     *            number of the batch
     * @param encryptedBallots
     *            Ballots before being mixed
     * @param reencryptedBallots
     *            Ballots mixed and reencrypted
     * @return Shuffle proof verifier configured with the given parameters
     * @throws IOException
     *             If files were not accessible or have problems when reading it
     */
