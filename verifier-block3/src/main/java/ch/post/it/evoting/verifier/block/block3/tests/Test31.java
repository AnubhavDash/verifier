...
public class Test31 extends Test {
    ...
    public TestDefinition getTestDefinition() {
        ...
        testDefinition.setId(31);
        testDefinition.setName("checkCiphertextConsistencyOnlineProofs");
        testDefinition.setDescription(TranslationHelper.getFromResourceBundle(Block3TestSuite.RESOURCE_BUNDLE_NAME, "test31.description"));
                ...
                if (map.size() == 3) {
                    while (nbControlled != 3) {
                        ...
                        for (Map.Entry<String, Tuple2<List<GammaPhis>, List<GammaPhis>>> tuple : map.entrySet()) {
                            ...
                            nbNotFound++;
                            if (nbNotFound == 3) {
                                throw new TestFailureException("Same vote not exist (vote non confirmé)");
                            }
                        }
                    }
                } else {
                    throw new TestFailureException("There aren't 3 online control component proof");
                }
            }
