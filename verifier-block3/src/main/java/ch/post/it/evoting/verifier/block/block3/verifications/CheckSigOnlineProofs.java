	...
	static class OnlineProofSignatureSourceBuilder {
		static String build(OnlineMixing onlineMixing, ShuffleProof shuffleProof) {
			...
			String voteEnc = String.format("{\"publicKey\":{\"zpSubgroup\":{\"g\":\"%s\",\"p\":\"%s\",\"q\":\"%s\"},\"elements\":[%s]}}",
					...
					String.join(",", onlineMixing.getVoteEncryptionKey().getElements().stream()
							.map(b -> String.format("\"%s\"", TypeConverter.bigIntegerToB64String(b))).collect(Collectors.toList()))
			);
			...
			String prevVoteEnc = String.format("{\"publicKey\":{\"zpSubgroup\":{\"g\":\"%s\",\"p\":\"%s\",\"q\":\"%s\"},\"elements\":[%s]}}",
					...
					String.join(",", onlineMixing.getPreviousVoteEncryptionKey().getElements().stream()
							.map(b -> String.format("\"%s\"", TypeConverter.bigIntegerToB64String(b))).collect(Collectors.toList()))
			);
		...
		private static String serialize(ShuffleProof proof) {
			return String
					.format("{\n  \"initialMessage\" : [ %s ],\n  \"firstAnswer\" : [ %s ],\n  \"secondAnswer\" : {\n    \"msgPA\" : {\n      \"commitmentPublicB\" : %s,\n      \"iniSVA\" : {\n        \"commitmentPublicD\" : %s,\n        \"commitmentPublicLowDelta\" : %s,\n        \"commitmentPublicHighDelta\" : %s\n      },\n      \"ansSVA\" : {\n        \"exponentsTildeA\" : [ %s ],\n        \"exponentsTildeB\" : [ %s ],\n        \"exponentTildeR\" : %s,\n        \"exponentTildeS\" : %s\n      },\n      \"iniHPA\" : {\n        \"commitmentPublicB\" : [ %s ]\n      },\n      \"ansHPA\" : {\n        \"initial\" : {\n          \"commitmentPublicA0\" : %s,\n          \"commitmentPublicBM\" : %s,\n          \"commitmentPublicD\" : [ %s ]\n        },\n        \"answer\" : {\n          \"exponentsA\" : [ %s ],\n          \"exponentsB\" : [ %s ],\n          \"exponentR\" : %s,\n          \"exponentS\" : %s,\n          \"exponentT\" : %s\n        }\n      }\n    },\n    \"iniMEBasic\" : {\n      \"commitmentPublicA0\" : %s,\n      \"commitmentPublicB\" : [ %s ],\n      \"ciphertextsE\" : [ %s ]\n    },\n    \"ansMEBasic\" : {\n      \"exponentsA\" : [ %s ],\n      \"exponentR\" : %s,\n      \"exponentsB\" : %s,\n      \"exponentS\" : %s,\n      \"randomnessTau\" : %s\n    }\n  }\n}",
							...
							Arrays.stream(proof.getSecondAnswer().getMsgPA().getAnsSVA().getExponentsTildeA()).map(e -> serializeWithOneIndent(e))
									.collect(Collectors.joining(", ")),
							Arrays.stream(proof.getSecondAnswer().getMsgPA().getAnsSVA().getExponentsTildeB()).map(e -> serializeWithOneIndent(e))
									.collect(Collectors.joining(", ")),
							...
							Arrays.stream(proof.getSecondAnswer().getMsgPA().getIniHPA().getCommitmentPublicB())
									.map(pc -> serializeWithTwoIndents(pc)).collect(Collectors.joining(", ")),
							...
							Arrays.stream(proof.getSecondAnswer().getMsgPA().getAnsHPA().getInitial().getCommitmentPublicD())
									.map(pc -> serializeWithThreeIndents(pc)).collect(Collectors.joining(", ")),
							Arrays.stream(proof.getSecondAnswer().getMsgPA().getAnsHPA().getAnswer().getExponentsA())
									.map(e -> serializeWithTwoIndents(e)).collect(Collectors.joining(", ")),
							Arrays.stream(proof.getSecondAnswer().getMsgPA().getAnsHPA().getAnswer().getExponentsB())
									.map(e -> serializeWithTwoIndents(e)).collect(Collectors.joining(", ")),
							...
							Arrays.stream(proof.getSecondAnswer().getIniMEBasic().getCommitmentPublicB()).map(pc -> serializeWithOneIndent(pc))
									.collect(Collectors.joining(", ")),
							Arrays.stream(proof.getSecondAnswer().getIniMEBasic().getCiphertextsE()).map(c -> serialize(c))
									.collect(Collectors.joining(", ")),
							Arrays.stream(proof.getSecondAnswer().getAnsMEBasic().getExponentsA()).map(e -> serialize(e))
									.collect(Collectors.joining(", ")),
		...
		private static String serialize(Ciphertext c) {
			return String.format("{\n        \"gamma\" : \"%s\",\n        \"phis\" : \"%s\"\n      }",
					...
					Arrays.asList(c.getParts()).subList(1, c.getParts().length).get(0).getValue() + ";" + c.getParts()[0].getParams()
							.getP() + ";" + c.getParts()[0].getParams().getOrder());
		}
