...
public class CheckCipherTextConsistencyOfflineProofs extends AbstractVerification {
	...
	public VerificationDefinition getVerificationDefinition() {
		...
		verificationDefinition
				.setDescription(TranslationHelper.getFromResourceBundle(Block3VerificationSuite.RESOURCE_BUNDLE_NAME, "verification11.description"));
	...
	public VerificationResult verify(Path inputDirectoryPath) throws Exception {
		...
		for (Path ballotBoxIdDirectoryPath : ballotBoxIdDirectoriesPathNode.getRegexPaths()) {
			...
			for (Path ballotBoxDirectoryPath : ballotBoxOfflineDirectoriesPathNode.getRegexPaths()) {
				OfflineEncryptedBallotsLoader offlineEncryptedBallotsLoader = new OfflineEncryptedBallotsLoader(ballotBoxDirectoryPath,
						inputDirectoryPath);
	...
	private boolean isOfflineOnlineEncryptedBallotsEquals(List<ElGamalEncryptedBallot> offlineList, List<ElGamalEncryptedBallot> onlineList) {
		return offlineList.size() == onlineList.size() && Flux.fromIterable(offlineList).zipWith(Flux.fromIterable(onlineList))
				.all(t -> t.getT1().equals(t.getT2())).block();
	}
}
