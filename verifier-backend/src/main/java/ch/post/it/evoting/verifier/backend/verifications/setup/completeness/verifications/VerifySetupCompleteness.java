 ...
 */
package ch.post.it.evoting.verifier.backend.verifications.setup.completeness.verifications;
...
public class VerifySetupCompleteness extends AbstractVerification {
	...
	public VerificationDefinition getVerificationDefinition() {
		...
		definition.setDescription(
				TranslationHelper.getFromResourceBundle(SetupVerificationSuite.RESOURCE_BUNDLE_NAME, "verification00.description"));
		definition.setId(0);
		definition.setName("verifySetupCompleteness");
		definition.addVerifierEvent(PreConfigurationEvent.TYPE);
		return definition;
	}
	...
	public VerificationResult verify(Path inputDirectoryPath) {
		if (verifySetupCompleteness(inputDirectoryPath)) {
			...
			return VerificationResult.failure(getVerificationDefinition(),
					TranslationHelper.getFromResourceBundle(SetupVerificationSuite.RESOURCE_BUNDLE_NAME, "verification00.nok.message"));
		}
	}
	...
	private boolean verifySetupCompleteness(final Path inputDirectoryPath) {
		try {
			...
			final List<Path> verificationCardSetIds = pathService.buildFromRootPath(StructureKey.VERIFICATION_CARD_SET_ID_DIR, inputDirectoryPath)
					.getRegexPaths();
			verificationCardSetIds.forEach(vcs -> pathService.buildFromDynamicAncestorPath(StructureKey.CONTROL_COMPONENT_CODE_SHARES, vcs));
			verificationCardSetIds.forEach(vcs -> pathService.buildFromDynamicAncestorPath(StructureKey.SETUP_COMPONENT_TALLY_DATA, vcs));
			verificationCardSetIds.forEach(vcs -> pathService.buildFromDynamicAncestorPath(StructureKey.SETUP_COMPONENT_VERIFICATION_DATA, vcs));
			return true;
