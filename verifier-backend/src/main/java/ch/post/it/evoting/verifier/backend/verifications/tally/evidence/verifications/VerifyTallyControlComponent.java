...
import ch.post.it.evoting.verifier.backend.verifications.tally.evidence.algorithms.VerifyTallyControlComponentBallotBoxAlgorithm;
import ch.post.it.evoting.verifier.backend.verifications.tally.evidence.algorithms.VerifyTallyControlComponentBallotBoxContext;
import ch.post.it.evoting.verifier.backend.verifications.tally.evidence.algorithms.VerifyTallyControlComponentBallotBoxInput;
...
public class VerifyTallyControlComponent extends AbstractVerification {
	...
	public VerificationDefinition getVerificationDefinition() {
		...
		definition
				.setDescription(getFromResourceBundle(TallyVerificationSuite.RESOURCE_BUNDLE_NAME, "tally.verification11.description"));
		definition.setName("verifyTallyControlComponents");
	...
	public VerificationResult verify(Path inputDirectoryPath) {
		...
		final List<Boolean> tallyVerif = IntStream.range(0, N_bb)
				.mapToObj(i -> {
					...
					final VerifyTallyControlComponentBallotBoxContext context = new VerifyTallyControlComponentBallotBoxContext(ee, bb_i);

					final Integer numberOfAllowedWriteIns = verificationCardSetContexts.stream()
							.filter(vcsContext -> vcsContext.ballotBoxId().equals(bb_i))
					...
					final VerifyTallyControlComponentBallotBoxInput input = new VerifyTallyControlComponentBallotBoxInput.Builder()
							...
							.setEncodedVotingOptions(p_tilde)
							.setNumberOfSelectableVotingOptions(numberOfSelections)
							.setNumberOfAllowedWriteIns(numberOfAllowedWriteIns)
		...
		if (tallyVerif.stream().allMatch(Boolean::booleanValue)) {
			...
			return VerificationResult.failure(getVerificationDefinition(),
					getFromResourceBundle(TallyVerificationSuite.RESOURCE_BUNDLE_NAME, "tally.verification11.nok.message"));
		}
	}
}
