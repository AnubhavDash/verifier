...
public class CertificateLoader {
	...
	public byte[] loadBytes(final StructureKey structureKey, final Path inputDirectoryPath) {
		final PathNode certificatePathNode = pathService.buildFromRootPath(structureKey, inputDirectoryPath);
	...
	private X509Certificate parseCertificate(byte[] certificate) {
		...
		final PEMParser parser = new PEMParser(new BufferedReader(new InputStreamReader(new ByteArrayInputStream(certificate))));
		try {
