...
import ch.post.it.evoting.verifier.backend.verifications.setup.evidence.algorithms.VerifyEncryptionParametersAlgorithm;
...
public class VerifyEncryptionParameters extends AbstractVerification {
	...
	public VerificationDefinition getVerificationDefinition() {
		...
		definition.setDescription(
				TranslationHelper.getFromResourceBundle(SetupVerificationSuite.RESOURCE_BUNDLE_NAME, "setup.verification01.description"));
		definition.setId(1);
	...
	public VerificationResult verify(final Path inputDirectoryPath) {
		...
		if (!verifyEncryptionParametersAlgorithm.verifyEncryptionParameters(p_hat, q_hat, g_hat, seed)) {
			verificationResult = VerificationResult.failure(getVerificationDefinition(),
					TranslationHelper.getFromResourceBundle(SetupVerificationSuite.RESOURCE_BUNDLE_NAME, "setup.verification01.nok.message"));
		} else {
