...
class VerifyConfirmedEncryptedVotesConsistencyTest extends TallyVerificationTest {
	...
	@Test
	void testVerifyNok() {
		final List<ControlComponentBallotBoxPayload> controlComponentBallotBoxPayloads = extractionService.getControlComponentBallotBoxPayloads(
				datasetPath);
		...
		final VerificationResult expectedResult = VerificationResult.failure(verificationWithMock.getVerificationDefinition(), getFromResourceBundle(
				TallyVerificationSuite.RESOURCE_BUNDLE_NAME, "tally.verification41.nok.message"));
		assertEquals(expectedResult, verificationResult);
	}
}