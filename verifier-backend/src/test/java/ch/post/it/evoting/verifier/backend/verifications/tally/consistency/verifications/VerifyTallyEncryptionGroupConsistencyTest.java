...
class VerifyTallyEncryptionGroupConsistencyTest extends TallyVerificationTest {
	...
	static void setupAll() {
		...
		verification = new VerifyTallyEncryptionGroupConsistency(applicationEventPublisherMock, extractionService);
	}
	...
	void invalidControlComponentBallotBoxPayload() {
		...
		final Stream<EncryptionGroupParameters> controlComponentPublicKeysParameters = extractionService.getFromControlComponentBallotBoxPayloads(
				datasetPath);
	...
	void invalidControlComponentShufflePayload() {
		final EncryptionGroupParametersExtractionService groupParametersExtractorMock = spy(extractionService);
		final Stream<EncryptionGroupParameters> controlComponentPublicKeysParameters = extractionService.getFromControlComponentShufflePayloads(
				datasetPath);
		doReturn(Streams.concat(Stream.of(differentEncryptionGroupParameters), controlComponentPublicKeysParameters)).when(
	...
	void invalidTallyComponentShufflePayload() {
		final EncryptionGroupParametersExtractionService groupParametersExtractorMock = spy(extractionService);
		doReturn(differentEncryptionGroupParameters).when(groupParametersExtractorMock).getFromTallyComponentShufflePayload(datasetPath);
	...
	void invalidTallyComponentVotesPayload() {
		final EncryptionGroupParametersExtractionService groupParametersExtractorMock = spy(extractionService);
		doReturn(differentEncryptionGroupParameters).when(groupParametersExtractorMock).getFromTallyComponentVotesPayload(datasetPath);

		assertInvalidVerification(groupParametersExtractorMock);
	}

	private void assertInvalidVerification(final EncryptionGroupParametersExtractionService groupParametersExtractorMock) {
		final VerifyTallyEncryptionGroupConsistency invalidVerification = new VerifyTallyEncryptionGroupConsistency(applicationEventPublisherMock,
		...
		final VerificationResult expectedResult = VerificationResult.failure(invalidVerification.getVerificationDefinition(),
				TranslationHelper.getFromResourceBundle(TallyVerificationSuite.RESOURCE_BUNDLE_NAME, "tally.verification30.nok.message"));
		assertEquals(expectedResult, verificationResult);
	}
}