...
public performRelease(projectName, gitUrl, branchName, autoMerge, mavenGoals, mavenParameters, workspace, releaseVersion, newSnapshotVersion, releaseEmail, useArtifactoryArtifactUploader, buildInfoArtifactName, releaseMessage) {
	...
	stage('update version') {
		...
		commonBuildPipeline.LOGGER('service version', "set new service version ${releaseVersion} in package.json!")
	...
	stage('set/build snapshot') {
		...
		try {
			...
			commonBuildPipeline.LOGGER("build", "we did not start a SNAPSHOT build, because no job was found for - project-name: '${projectName}' and/or branch-name: '${branchName}'")
		}
...
@NonCPS
private static String substring(String regexp, String payload) {
	Matcher match = Pattern.compile(regexp).matcher(payload)
	if (match.find()) {
